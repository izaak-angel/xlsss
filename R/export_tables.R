  ...
  tables %>%
    ...
    purrr::pmap(function(table, start_row, end_row, table_title, table_name) {
      xlsss::add_data_table(wb,
                            ...
                            header_rows,
                            table_name)
      if(nrow(tables) > 1 | table_headings){
...
#' @param use_low Does the page use low for non-sensitive values
#' @param table_headings Use subheadings for all tables
#' @export
add_data_sheet <- function(wb,
  ...
  if(use_low){
    openxlsx::writeData(wb, sheet_name,
                      x = "[low] indicates that a figure is less than three and greater than zero.",
                      startRow = 6,
                      startCol = 1
                      )
  }
...
negative_to_c <- function (wb,
  ...
  if (table[[column]][[row]] == -100 & (!is.na(table[[column]][[row]]))){
    openxlsx::writeData(wb,
                        sheet_name,
                        x = "[low]",
                        startCol = column,
...
#' @param notes_list List of notes
#' @param use_low Does the wb use low for non-sensitive values
#' @param table_headings Use subheadings for all tables
#' @export
add_data_sheets <- function(wb, table_layout, notes_list, use_low = FALSE, table_headings = FALSE) {
  purrr::walk(
    1:nrow(table_layout),
